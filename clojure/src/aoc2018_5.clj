...
(defn removing-pair
  "반응하는 쌍을 삭제해준다 삭제를 순환하며 계속하다가 원래 문자열과 삭제된 문자열이 같다면 모두 삭제된것이라 판단한다."
  [{:keys [mentioned origin]}]
  (let [removed (reduce remove-pair origin mentioned)]
    (if (= removed origin)
      removed
      (removing-pair {:mentioned mentioned :origin removed}))))
...
(def push-not-pair
  ...
  (fn [stack char]
    (let [last-char-in-stack (peek stack)]
      (if (and (not= last-char-in-stack nil)
...
(comment
  (length-polymer "dabAcCaCBAcCcaDA"))
...
(defn input->removed-units
  ...
  [input]
  (let [mentioned (set (map clojure.string/lower-case input))]
    (map remove-char mentioned (cycle [input]))))

(defn removed-units->count-polymer
  "각 문자열들에서 반응이후의 글자갯수를 가져온다"
  [strings]
  (map length-polymer strings))

(comment
  (->> "dabAcCaCBAcCcaDA"
       input->removed-units
       removed-units->count-polymer
       (apply min)))
